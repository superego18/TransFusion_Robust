...
class TransFusionHead(nn.Module):
    ...
    def forward_single(self, inputs, img_inputs, img_metas):
        ...
        if self.initialize_by_heatmap:
            ...
            ## reduce nms radius for smaller objects
            if self.test_cfg['dataset'] == 'nuScenes':
            ...
            elif self.test_cfg['dataset'] == 'Robust':  # for 'pedestrian', 'motorcycle', 'bicycle'
                ...
                local_max[:, 6, ] = F.max_pool2d(heatmap[:, 6], kernel_size=1, stride=1, padding=0)
    ...
    def get_bboxes(self, preds_dicts, img_metas, img=None, rescale=False, for_roi=False):
        ...
        for layer_id, preds_dict in enumerate(preds_dicts):
            ...
            elif self.test_cfg['dataset'] == 'Robust':
                self.tasks = [
                    dict(num_class=7, class_names=[], indices=[0, 1, 2, 3], radius=-1),
                    dict(num_class=1, class_names=['pedestrian'], indices=[4], radius=0.175),
                    dict(num_class=1, class_names=['motocycle'], indices=[5], radius=0.175),
                    dict(num_class=1, class_names=['bicycle'], indices=[6], radius=0.175),
                ]
