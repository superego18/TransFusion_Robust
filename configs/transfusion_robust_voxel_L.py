...
train_pipeline = [ # Data preprocssig pipeline before training
    ...
    dict(type='ObjectSample', # Class-balanced Grouping and Sampling (CBGS)
         db_sampler=dict( # GT db and info on the internal disk
             data_root='/home/chanju/transfusion/TransFusion/data',
             info_path='/home/chanju/transfusion/TransFusion/data/robust_dbinfos_train.pkl',
             rate=1.0,
             prepare=dict(
             ...
             classes=class_names,
             sample_groups=dict( # Groups similar in size and shape
                car=2, # group 1 (12.1 = total_car_gt / total_sample)
                truck=6, # group 2 (2.8)
                other_vehicles=6, # group 2 (0.14)
                bus=12, # group 3 (0.97)
                pedestrian=4, # group 4 (8.4)
                motorcycle=6, # group 5 (0.92)
                bicycle=6 # group 5 (0.09)
                ), 
             points_loader=dict(
    ...
    dict(
        type='GlobalRotScaleTrans', # Sometimes the order is changed from RandomFlip3D.
        rot_range=[-0.78539816, 0.78539816], # Based on Waymo, However in the paper, this is stated as [-pi/8, pi/8].
        scale_ratio_range=[0.95, 1.05], # Based on nuScenes, However in the paper, this is stated as [0.9, 1.1].
        translation_std=[0.5, 0.5, 0.5]), # Based on nuScenes
    dict(
        type='RandomFlip3D',
        sync_2d=False,
    ...
    dict(type='DefaultFormatBundle3D', class_names=class_names), # Convert data(points, gt_labels_3d, gt_bboxes_3d) to DataContainer of mmcv. 
    dict(type='Collect3D', keys=['points', 'gt_bboxes_3d', 'gt_labels_3d']) # Seperate to keys(points, gt_labels_3d, gt_bboxes_3d) and img_metas(others). # All are DC.
test_pipeline = [
...
data = dict(
    samples_per_gpu=3, # Support up to 3 on RTX 3080. (Because of CUDA OOM in multi_head_attention_forward.)
...
work_dir = None
load_from = None
resume_from = None # If you want to continue the epoch, use this.
workflow = [('train', 1)]
gpu_ids = range(0, 8)
