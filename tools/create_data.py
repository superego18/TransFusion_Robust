...
from tools.data_converter import robust_converter as robust_converter
from tools.data_converter import nuscenes_converter as nuscenes_converter
from tools.data_converter.create_gt_database import create_groundtruth_database
from tools.data_converter.create_gt_database_for_robust import create_groundtruth_database_for_robust
...
def robust_data_prep():
    
    # create robust data info file
    split_list = ['train', 'val', 'test']
    # split_list = ['train-mini']
    data_root = '/mnt/e/Robust/transfusion'
    
    for split in split_list:
        
        # get idx list
        split_txt_path = f'{data_root}/split/{split}.txt'
        with open(split_txt_path, 'r') as f:
            idx_list = [line.strip() for line in f.readlines()]
                        
        pkl_file_path = f'{data_root}/robust_infos_{split}.pkl'
        
        print(f'make robust_infos_{split}.pkl having {len(idx_list)} samples')
        info_list = Parallel(n_jobs=-1)(delayed(robust_converter.create_robust_infos)(idx) for idx in tqdm(idx_list))
        with open(pkl_file_path, 'wb') as file:
            pickle.dump(info_list, file)

    # create robust gt db info file & gt database
    create_groundtruth_database_for_robust(
        'RobustDataset',
        data_root,
        f'{data_root}/robust_infos_train.pkl'
        )
